...
const Label = ({label, htmlFor}) => <label htmlFor={htmlFor}
                                           className="form-label">{label}</label>;

export const FormError = ({error}) => error ? <p
    className="form-error">{error}</p> : null;
...
export const MultiSelect = ({label, name, value, error, className, options = [], ...props}) => {
    return (
        <Field type='select' error={error}>
            <Label label={label} htmlFor={name}/>
            <select defaultValue={value} {...props} id={name} name={name}
                    className={[
                        'form-select',
                        error ? 'form-error' : '',
                        className].join(' ')}>
...
export const Switch = ({value = false, label, name, error, onChange, onBlur, className, ...props}) => {
    ...
    return (
        <Field type='switch' error={error}>
            <Label label={label} htmlFor={name}/>
            <button type='button' id={name} title={title} onBlur={() => {
                onBlur(true);
            }} onClick={() => {
                onChange(name, !value);
            }} className={classes}
...
export const MySlider = ({label, value, name, type, error, decimals = 0, className, onChange, onBlur, ...props}) => {
    return (
        <Field type={type} error={error}>
            <Label label={label + ': ' + value.toFixed(decimals)}
                   htmlFor={name}/>
            <StyledSlider name={name} id={name} value={value}
